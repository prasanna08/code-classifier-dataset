...
class CodeClassifier(object):
    ...
    def _cv_tokenizer(self, program):
        """Tokenize Python program for CountVectorizer."""
        token_program = []
        for tid, tname in self._get_tokens(program):
    ...
    def _tokenize_data(self, data, threshold=5):
        ...
        # Add 'UNK' in token_to_id. This will be used to replace any token occurring
        # in program which is not in valid_token.
        token_to_id['UNK'] = len(token_to_id)

        # Add 'V' in token_to_id. This token will be used to replace all variables
        # and methods in program.
    ...
    def _hash_generator(self, token_to_id, tokens):
        """Generate hash for tokens in 'tokens' using token_to_id."""
        hash_val = 0
        n = len(tokens) - 1
        for token in tokens:
            hash_val += token_to_id[token] * (len(token_to_id) ** n )
            n -= 1
        return hash_val

    def _k_gram_hash_generator(self, token_program, token_to_id, K):
        """Generate all k-gram hashes for tokenized program."""
        generated_hashes = [
            self._hash_generator(token_to_id, token_program[i: i+K])
            for i in xrange(0, len(token_program) - K)]
        return generated_hashes

    def _generate_k_gram_hashes(self, data, token_to_id, K=3):
        """Generate k-gram hashes for all programs in dataset."""
        for program_id in data:
            data[program_id]['k_gram_hashes'] = self._k_gram_hash_generator(
                data[program_id]['tokens'], token_to_id, K)
        return data

    def _get_fingerprint_from_hashes(self, k_gram_hashes, window_size):
    ...
    def _generate_program_fingerprints(self, data, T, K):
        """Generate document fingerprints for all programs in entire dataset."""
        window_size = T - K + 1
        for program_id in data:
            data[program_id]['fingerprint'] = self._get_fingerprint_from_hashes(
                data[program_id]['k_gram_hashes'], window_size)
        return data    

    def _calc_jaccard_index(self, A, B):
    ...
    def _generate_top_similars(self, data, top):
        ...
        for program_id_1 in data:
            ...
            data[program_id_1]['top_similar'] = top_overlaps
        return data

    def _run_knn(self, data):
        ...
        for pid in data:
            ...
            if pid in similars:
                similars.remove(pid)
            nearest_classes = [data[i]['class'] for (i, s) in similars]
    ...
    def train(self, training_data):
        ...
        data, token_to_id = self._tokenize_data(data)
        ...
        for T in range(4, T_max):
            for K in range(3, T):
                data = self._generate_k_gram_hashes(data, token_to_id, K)
                data = self._generate_program_fingerprints(data, T, K)
                data = self._generate_top_similars(data, top)
                occurrence, missclassified_points = self._run_knn(data)
                accuracy = (
        ...
        cv = CountVectorizer(tokenizer=self._cv_tokenizer, min_df=5)
        cv.fit(programs)
    ...
    def validate(self, classifier_data):
        ...
        allowedd_knn_keys = ['T', 'K', 'top', 'occurrence',
                             'token_to_id', 'data']
        for key in allowedd_knn_keys:
    ...
    def get_classification_report(self):
        ...
        for pid in self.data:
            similars = self.data[pid]['top_similar']
            if pid in similars:
                similars.remove(pid)
            nearest_classes = [self.data[i]['class'] for (i, s) in similars]
            cnt = Counter(nearest_classes)
            common = cnt.most_common(1)
            if common[0][1] < self.occurrence:
                missclassified_points.append(pid)
            else:
                self.data[pid]['prediction'] = common[0][0]

        Xc = np.array(
            [self.data[pid]['vector'] for pid in missclassified_points])
        Xc = np.squeeze(Xc)

        SYp = self.clf.predict(Xc)
        for (i, pid) in enumerate(missclassified_points):
            self.data[pid]['prediction'] = SYp[i]

        # Predictions.
        Y = np.array([self.data[pid]['class'] for pid in self.data])
        Yp = np.array(
            [self.data[pid]['prediction'] for pid in self.data])

        print self._calc_metrics(Y, Yp)

if __name__ == '__main__':
    with open('formatted_training_data.json', 'r') as f:
        data = json.loads(f.read())

    clf = CodeClassifier()
    clf.train(data)
    classifier_data = clf.to_dict()
    clf.validate(classifier_data)
